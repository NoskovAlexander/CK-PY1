...
def to_csv_file(filename: str, headers: list[str], rows: list[list[str]], delimiter=",",
                new_line="\n"):  # TODO реализовать функцию to_csv_file
    with open(filename, "w") as drain:
        drain.write(delimiter.join(headers) + new_line)
        for row in rows:
            drain.write(delimiter.join(row) + new_line)
